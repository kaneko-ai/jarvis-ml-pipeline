...
def validate_submission(
    ...
    if list(df.columns) != [target_col]:
        raise ValueError(
            f"Column name mismatch. Expected: ['{target_col}'], " f"Got: {list(df.columns)}"
        )

    # 行数チェック
    if expected_rows is not None and len(df) != expected_rows:
        raise ValueError(f"Row count mismatch. Expected: {expected_rows}, " f"Got: {len(df)}")
    ...
    if expected_classes is not None:
        ...
        if not unique_values.issubset(expected_set):
            raise ValueError(
                f"Unexpected class values. Expected: {expected_set}, " f"Got: {unique_values}"
            )
