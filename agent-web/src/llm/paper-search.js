...
async function fetchWith429Retry(url, options = {}, timeoutMs = 15000, maxRetries = 2) {
  let lastError = null;
...
async function searchPubMed(query, maxResults = 5) {
  try {
    ...
    const searchUrl =
      PUBMED_BASE +
      "/esearch.fcgi?db=pubmed&retmode=json&retmax=" +
      maxResults +
      "&sort=relevance&term=" +
    ...
    let searchRes;
    try {
      searchRes = await fetchWith429Retry(searchUrl);
    } catch (e) {
      console.error("PubMed ESearch network error:", e.message);
      return [];
    }
    ...
    let summaryRes;
    try {
      summaryRes = await fetchWith429Retry(summaryUrl);
    } catch (e) {
      console.error("PubMed ESummary network error:", e.message);
      return [];
    }
...
async function searchSemanticScholar(query, maxResults = 5) {
  try {
    ...
    let res;
    try {
      res = await fetchWith429Retry(url, { headers });
    } catch (e) {
      console.error("Semantic Scholar network error:", e.message);
      return [];
    }
...
async function searchOpenAlex(query, maxResults = 5) {
  try {
    ...
    let res;
    try {
      res = await fetchWith429Retry(url, {
        headers: { "User-Agent": "JARVIS-ResearchOS/1.0 (mailto:jarvis@localhost)" },
      });
    } catch (e) {
      console.error("OpenAlex network error:", e.message);
      return [];
    }
