...
class EvidenceValidator:
    ...
    def __init__(self, doc_store: Optional[Dict[str, Paper]] = None):
        """
        初期化.
    ...
    def validate_evidence_link(
        self, 
        claim: Claim, 
        doc_store: Optional[Dict[str, Paper]] = None
    ) -> ValidationResult:
        """
        Claimの全evidenceを検証.
    ...
    def _get_section_text(self, paper: Paper, section: str) -> Optional[str]:
        """論文からセクションテキストを取得."""
        if section == "title":
            return paper.title
...
def get_evidence_validator(doc_store: Optional[Dict[str, Paper]] = None) -> EvidenceValidator:
