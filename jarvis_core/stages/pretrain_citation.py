...
def stage_get_paper_A(context: TaskContext, artifacts: Artifacts) -> Artifacts:
    ...
    artifacts.add_claim(Claim(
        ...
        evidence=[EvidenceLink(
            doc_id="internal", section="get_paper_A",
...
def stage_references_from_A(context: TaskContext, artifacts: Artifacts) -> Artifacts:
    ...
    artifacts.add_claim(Claim(
        ...
        evidence=[EvidenceLink(
            doc_id="internal", section="references_from_A",
            chunk_id="log", start=0, end=10, confidence=1.0
        )],
        claim_type="log"
    ))
    
    log_audit("extraction.references_from_A", "completed",
              details={"ref_count": len(references)})
    return artifacts
...
def stage_fetch_R_set(context: TaskContext, artifacts: Artifacts) -> Artifacts:
    ...
    artifacts.add_claim(Claim(
        ...
        evidence=[EvidenceLink(
            doc_id="internal", section="fetch_R_set",
            chunk_id="log", start=0, end=10, confidence=1.0
        )],
        claim_type="log"
    ))
    
    log_audit("retrieval.fetch_R_set", "completed",
              details={"docs_count": len(R_set_docs)})
    return artifacts
...
def stage_leakage_filter(context: TaskContext, artifacts: Artifacts) -> Artifacts:
    ...
    artifacts.add_claim(Claim(
        ...
        evidence=[EvidenceLink(
            doc_id="internal", section="leakage_filter",
            chunk_id="log", start=0, end=10, confidence=1.0
        )],
        claim_type="log"
    ))
    
    log_audit("screening.leakage_filter", "completed",
              details={"high_leakage": high_leakage_count})
    return artifacts
...
def stage_R_set_digest(context: TaskContext, artifacts: Artifacts) -> Artifacts:
    ...
    artifacts.add_claim(Claim(
        ...
        evidence=[EvidenceLink(
            doc_id="internal", section="R_set_digest",
            chunk_id="log", start=0, end=10, confidence=1.0
        )],
        claim_type="log"
    ))
...
def stage_reconstruct_background_discussion(context: TaskContext, artifacts: Artifacts) -> Artifacts:
    """
    R_digestから背景・考察を再構成.
    ...
    reconstruction = {
        ...
        "predicted_conclusions": [
            {
                "text": "Predicted conclusion",
                "prob": 0.65,
                "evidence": []
            }
        ],
    ...
    artifacts.add_claim(Claim(
        ...
        evidence=[EvidenceLink(
            doc_id="internal", section="reconstruct",
            chunk_id="log", start=0, end=10, confidence=1.0
        )],
        claim_type="log"
    ))
...
def stage_gold_from_A(context: TaskContext, artifacts: Artifacts) -> Artifacts:
    ...
    intro_text = paper_A.get("sections", {}).get("Introduction", 
                 paper_A.get("sections", {}).get("Background", ""))
    ...
    disc_text = paper_A.get("sections", {}).get("Discussion",
                paper_A.get("sections", {}).get("Conclusion", ""))
    ...
    artifacts.add_claim(Claim(
        ...
        evidence=[EvidenceLink(
            doc_id="internal", section="gold_from_A",
            chunk_id="log", start=0, end=10, confidence=1.0
        )],
        claim_type="log"
    ))
    
    log_audit("extraction.gold_from_A", "completed",
              details={"gold_count": len(gold_points)})
    return artifacts
...
def stage_match_score(context: TaskContext, artifacts: Artifacts) -> Artifacts:
    ...
    points_with_evidence = sum(
        1 for p in reconstruction.get("background_points", [])
        if p.get("evidence")
    )
    ...
    # Leakage penalty
    high_leakage_ratio = sum(
        1 for f in leakage_flags if f.get("leakage_level") == "high"
    ) / max(1, len(leakage_flags))
    leakage_penalty = high_leakage_ratio * 0.3
    
    # Total
    total = (coverage * 0.3 + faithfulness * 0.3 + novelty_gap * 0.1 + 
             structure * 0.2) - leakage_penalty
    ...
    artifacts.add_claim(Claim(
        ...
        evidence=[EvidenceLink(
            doc_id="internal", section="match_score",
            chunk_id="log", start=0, end=10, confidence=1.0
        )],
        claim_type="log"
    ))
    
    log_audit("evaluation.match_score", "completed",
...
def stage_no_leakage_gate(context: TaskContext, artifacts: Artifacts) -> Artifacts:
    ...
    artifacts.add_claim(Claim(
        ...
        evidence=[EvidenceLink(
            doc_id="internal", section="no_leakage",
            chunk_id="log", start=0, end=10, confidence=1.0
        )],
        claim_type="log"
    ))
    
    log_audit("quality_gate.no_leakage", "completed",
...
def stage_store_training_record(context: TaskContext, artifacts: Artifacts) -> Artifacts:
    ...
    artifacts.add_claim(Claim(
        ...
        evidence=[EvidenceLink(
            doc_id="internal", section="store_training",
            chunk_id="log", start=0, end=10, confidence=1.0
        )],
        claim_type="log"
    ))
    
    log_audit("ops.store_training_record", "completed",
              details={"saved": saved})
    return artifacts
