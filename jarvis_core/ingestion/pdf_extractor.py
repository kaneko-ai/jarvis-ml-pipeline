...
class HardenedPDFExtractor:
    ...
    def extract(self, path: Path) -> Tuple[str, List[Tuple[int, str]], Optional[PDFFailureInfo]]:
        ...
        if file_size_mb > self.max_file_size_mb:
            return "", [], PDFFailureInfo(
                str(path), "LIMIT_EXCEEDED", 
    ...
    def _extract_pymupdf(self, path: Path) -> Tuple[str, List[Tuple[int, str]], Optional[PDFFailureInfo]]:
        ...
        try:
            with fitz.open(path) as doc:
                ...
                if len(doc) > self.max_pages:
                    return "", [], PDFFailureInfo(str(path), "LIMIT_EXCEEDED", f"Page count {len(doc)} exceeds limit {self.max_pages}")
    ...
    def _extract_pdfplumber(self, path: Path) -> Tuple[str, List[Tuple[int, str]], Optional[PDFFailureInfo]]:
        ...
        try:
            with pdfplumber.open(path) as pdf:
                if len(pdf.pages) > self.max_pages:
                    return "", [], PDFFailureInfo(str(path), "LIMIT_EXCEEDED", f"Page count {len(pdf.pages)} exceeds limit {self.max_pages}")
