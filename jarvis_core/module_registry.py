...
def _init_registry():
    # Phase Ω modules
    register_module(
        "autonomous_loop", "research_os", "Autonomous research loop", file_path="autonomous_loop.py"
    )
    register_module(
        "cross_field", "research_os", "Cross-field innovation", file_path="cross_field.py"
    )
    register_module(
        "failure_simulator", "research_os", "Failure simulation", file_path="failure_simulator.py"
    )
    register_module(
    ...
    register_module(
        "grant_optimizer", "research_os", "Grant optimization", file_path="grant_optimizer.py"
    )
    register_module(
        "reviewer_persona", "research_os", "Reviewer personas", file_path="reviewer_persona.py"
    )
    register_module(
        "lab_optimizer", "research_os", "Lab resource optimization", file_path="lab_optimizer.py"
    )
    register_module(
        "career_planner", "research_os", "Career planning", file_path="career_planner.py"
    )
    ...
    register_module(
        "negative_results", "decision", "Negative results vault", file_path="negative_results.py"
    )
    register_module(
        "reproducibility_cert",
        "decision",
        "Reproducibility certification",
        file_path="reproducibility_cert.py",
    )
    register_module(
        "lab_to_startup", "decision", "Lab to startup translation", file_path="lab_to_startup.py"
    )
    register_module(
        "clinical_readiness",
        "decision",
        "Clinical translation readiness",
        file_path="clinical_readiness.py",
    )
    register_module(
        "kill_switch", "decision", "Kill switch recommendation", file_path="kill_switch.py"
    )
    register_module(
        "student_portfolio",
        "decision",
        "Student portfolio management",
        file_path="student_portfolio.py",
    )
    ...
    register_module(
        "literature_to_plan",
        "workflow",
        "Literature to research plan",
        file_path="workflows/canonical.py",
    )
    register_module(
        "plan_to_grant", "workflow", "Plan to grant proposal", file_path="workflows/canonical.py"
    )
    register_module(
        "plan_to_paper", "workflow", "Plan to paper structure", file_path="workflows/canonical.py"
    )
    register_module(
        "plan_to_talk", "workflow", "Plan to presentation", file_path="workflows/canonical.py"
    )
