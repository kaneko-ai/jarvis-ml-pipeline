...
from .question_generator import QuestionGenerator, ResearchQuestion, QuestionType
from .action_planner import ActionPlanner, ActionPlan, ActionType, ActionItem, Executor
from .goldset_index import GoldsetIndex, GoldsetEntry
from .decision_item import DecisionStore, DecisionItem
...
class ResearchPartnerOutput:
    ...
    # Phase4
    key_questions: Optional[KeyQuestions] = None
    ...
    # Phase6
    strategic: Optional[StrategicSuggestion] = None
...
class ResearchPartner:
    ...
    def _derive_implication(self, entry: GoldsetEntry, theme: str) -> str:
        ...
        if entry.decision == "reject":
            return f"類似ケースでは見送りが正解だった。今回も慎重に。"
        else:
            return f"類似ケースでは採用が成功した。条件が合えば検討価値あり。"
    ...
    def _generate_key_questions(
        ...
        return KeyQuestions(
            ...
            implicit_assumption=f"この分野で「当たり前」とされている前提は妥当か？",
            decision_point=f"今ここで判断を誤ると後戻りが難しい選択は何か？",
        )
    ...
    def _create_action_plan(
        ...
        plan.read_items = [
            ...
            ActionItem(
                action_type=ActionType.READ,
                target=f"最新のbenchmark/evaluation論文",
