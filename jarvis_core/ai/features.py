...
class SimilarityCalculator:
    ...
    def compare_papers(self, paper1: dict, paper2: dict) -> dict:
        ...
        return {
            ...
            "similarity_level": (
                "High" if similarity > 0.3 else "Medium" if similarity > 0.15 else "Low"
            ),
        }
...
class KeywordExtractor:
    ...
    def extract(self, text: str, top_n: int = 10) -> list[tuple[str, int]]:
        ...
        sorted_keywords = sorted(
            counter.items(), key=lambda item: (-item[1], first_positions[item[0]], item[0])
        )
...
class CitationGenerator:
    ...
    def generate(self, paper: dict, format: str = "apa") -> str:
        ...
        if format == "apa":
            ...
            key = re.sub(
                r"[^a-zA-Z0-9]", "", authors.split(",")[0].split()[0] if authors else "unknown"
            ) + str(year)
