...
class TestGenerate:
    def test_gen_empty(self):
        from jarvis_core.analysis.review_generator import ReviewGenerator
        g = ReviewGenerator()
        if hasattr(g, 'generate'): g.generate([])
    
    def test_gen_1(self):
        from jarvis_core.analysis.review_generator import ReviewGenerator
        g = ReviewGenerator()
        if hasattr(g, 'generate'): g.generate([{"title":"P1"}])
    
    def test_gen_2(self):
        from jarvis_core.analysis.review_generator import ReviewGenerator
        g = ReviewGenerator()
        if hasattr(g, 'generate'): g.generate([{"title":"P2"}])
    
    def test_gen_3(self):
        from jarvis_core.analysis.review_generator import ReviewGenerator
        g = ReviewGenerator()
        if hasattr(g, 'generate'): g.generate([{"title":"P3"}])
