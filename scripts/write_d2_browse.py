...
Fallback: Jina Reader API when Scrapling extraction fails.
...
HEADERS = {
    "User-Agent": "JARVIS Research OS/2.0 (Academic research tool)",
...
# ========== Jina Reader API (D2-2 fallback) ==========

def _jina_reader(url: str, timeout: int = 30) -> dict:
    """Convert URL to structured content via Jina Reader API."""
    try:
        jina_url = f"https://r.jina.ai/{url}"
        headers = {
            "Accept": "application/json",
            "User-Agent": "JARVIS Research OS/2.0",
        }
        ...
        # Try to extract abstract from first 500 chars of content
        content = result.get("content", "")
        if content and len(content) > 50:
            result["abstract"] = content[:500].strip()
        return result
...
def _extract_pubmed(url: str, html: str) -> dict:
    ...
    # Abstract - try multiple selectors (D2-1 fix)
    abs_el = _first(page, "div.abstract-content")
    if abs_el is None:
        abs_el = _first(page, "#eng-abstract")
    if abs_el is None:
        abs_el = _first(page, "#abstract")
    if abs_el is None:
        abs_el = _first(page, "div.abstract")
    if abs_el:
    ...
    # Fallback: if abstract is empty, try Jina Reader
    if not meta.get("abstract"):
        jina = _jina_reader(url, timeout=15)
        if jina.get("abstract"):
            meta["abstract"] = jina["abstract"]
            meta["abstract_source"] = "jina_reader"

    # Authors - deduplicated (D2-1 fix)
    author_els = page.css("span.authors-list-item a.full-name")
    if author_els and len(author_els) > 0:
        raw = [a.text.strip() for a in author_els if a.text.strip()]
        meta["authors"] = _dedup_authors(raw)
    else:
        author_els2 = page.css("a.full-name")
        if author_els2 and len(author_els2) > 0:
            raw = [a.text.strip() for a in author_els2 if a.text.strip()]
            meta["authors"] = _dedup_authors(raw)
...
def _extract_arxiv(url: str, html: str) -> dict:
    ...
    if authors_div:
        ...
        if a_tags and len(a_tags) > 0:
            meta["authors"] = _dedup_authors([a.text.strip() for a in a_tags if a.text.strip()])
    if "authors" not in meta:
        author_meta = page.css('meta[name="citation_author"]')
        if author_meta and len(author_meta) > 0:
            meta["authors"] = _dedup_authors([m.attrib.get("content", "").strip() for m in author_meta])
...
def _extract_generic(url: str, html: str) -> dict:
    ...
    # Fallback: Jina Reader for generic pages with no abstract
    if not meta.get("abstract"):
        jina = _jina_reader(url, timeout=15)
        if jina.get("abstract"):
            meta["abstract"] = jina["abstract"]
    ...
    if author_tags and len(author_tags) > 0:
        meta["authors"] = _dedup_authors([t.attrib.get("content", "").strip() for t in author_tags])
...
def extract_metadata(url: str, timeout: int = 20) -> dict:
    ...
    try:
        ...
        print(f"  Scrapling fetch failed, trying Jina Reader...")
        jina = _jina_reader(url, timeout=30)
        if "error" not in jina and jina.get("title"):
            return jina
    ...
    for i, url in enumerate(urls, 1):
        ...
        if json_mode:
            ...
            if meta.get("abstract_source"):
                print(f"  Abstract: (via {meta['abstract_source']})")
